...
```{r traceplot jags1}
traceplot(jags_fit, varname = c("sigma2"), ask = F)
```
...
* 5.4 Ajusta un nuevo modelo utilizando una iniciales Beta(10,10) y Gamma(0.51,0.01)G para $\mu$ y $\kappa$ (lo demás quedará igual). Realiza una gráfica con las medias posteriores de los parámetros θj bajo los dos escenarios de distribuciones iniciales. En el eje horizontal grafica las medias posteriores del modelo ajustado en 6.2 y en el eje vertical las medias posteriores del modelo modelo en 6.4. ¿Cómo se comparan? 
